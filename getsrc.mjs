...
const FILE = process.env.FILE || "::/Demo/Games/Talons.HC.Z";
const OUT = process.env.OUT || "/tmp/talons.HC";
dv().setBigUint64(gBase + MBOX, 0n, true);
typeStr(`I64 sz_;U8 *s_=FileRead("${FILE}",&sz_);MemCpy(0x${(MBOX+16).toString(16).toUpperCase()},s_,sz_);*(0x${MBOX.toString(16).toUpperCase()})(I64*)=sz_;Free(s_);\n`);
run(400);
const sz = Number(dv().getBigUint64(gBase + MBOX, true));
console.error("decompressed size = " + sz);
if (sz > 0 && sz < 4000000) { const m = new Uint8Array(inst.exports.memory.buffer); writeFileSync(OUT, Buffer.from(m.subarray(gBase + MBOX + 16, gBase + MBOX + 16 + sz))); console.error("wrote " + OUT); }
else console.error("bad size; last screen:\n" + screenText().split("\n").slice(-6).join("\n"));
process.exit(0);
